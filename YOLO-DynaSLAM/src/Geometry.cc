...
void Geometry::GeometricModelCorrection(const ORB_SLAM2::Frame &currentFrame,
    ...
    if(currentFrame.mTcw.empty()){
        ...
        std::cout << "inside geometry model correction: get ref frames." << std::endl;
        vector<ORB_SLAM2::Frame> vRefFrames = GetRefFrames(currentFrame);
        std::cout << "extract dynamic points" << std::endl;
        vector<DynKeyPoint> vDynPoints = ExtractDynPoints(vRefFrames,currentFrame);
        std::cout << "mask depth region growing." << std::endl;
        mask = DepthRegionGrowing(vDynPoints,imDepth);
        std::cout << "geometry model correction done" << std::endl;
...
cv::Mat Geometry::DepthRegionGrowing(const vector<DynKeyPoint> &vDynPoints,const cv::Mat &imDepth){
    ...
    {
        mSegThreshold = 0.20;
//        std::cout << "mask depth region growing: before for loop" << vDynPoints.size() << std::endl;
        ...
        for (size_t i(0); i < vDynPoints.size(); i++){
            ...
            {
//        	std::cout << "mask depth region growing: inside for loop: before" << std::endl;
                cv::Mat J = RegionGrowing(imDepth,xSeed,ySeed,mSegThreshold);
                maskG = maskG | J;
//        	std::cout << "mask depth region growing: inside for loop: after" << std::endl;
            }
        }
//        std::cout << "mask depth region growing: after for loop" << std::endl;

        int dilation_size = 15;
        cv::Mat kernel = getStructuringElement(cv::MORPH_ELLIPSE,
                                               cv::Size( 2*dilation_size + 1, 2*dilation_size+1 ),
                                               cv::Point( dilation_size, dilation_size ) );
//        std::cout << "mask depth region growing: structural elements" << std::endl;
        maskG.cv::Mat::convertTo(maskG,CV_8U);
        cv::dilate(maskG, maskG, kernel);
    }
    else
    {
//        std::cout << "mask depth region growing: vdynpoints empty" << std::endl;
        maskG.cv::Mat::convertTo(maskG,CV_8U);
    }
...
void Geometry::file_func(){

	myfile.open ("SLAM_stats.txt");
}
...
cv::Mat Geometry::RegionGrowing(const cv::Mat &im,int &x,int &y,const float &reg_maxdist){
    ...
    double begin_time = std::chrono::duration_cast<std::chrono::microseconds>(current_time.time_since_epoch()).count()/1e6;
// 	myfile << begin_time ;
//  	myfile << " " ;
        ...
        //std::cout << "RegionGrowing: add new block of memory" << std::endl;
        ...
        //std::cout << "RegionGrowing: add new block of memory: done" << std::endl;
...
//    std::cout << "RegionGrowing: while counting " << while_count << std::endl;
//    std::cout << "RegionGrowing: reg_size counting " << reg_size << std::endl;
//    std::cout << "RegionGrowing: while counting " << while_count << std::endl;
