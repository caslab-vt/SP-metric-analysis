...
double RRT::randomObstacles(int num_obstacle, float x_min, float x_max, float y_min, float y_max)
{
	double obs_den=0;
...
//sched_setaffinity(getpid(),sizeof(cpuset_sched),&cpuset_sched);
  ...
  printf("\n\n**===-------------------------------------------------===**\n");
  float start[] = {0,0};
  float goal[] = {100,100};//{float(rand()%1000),float(rand()%1000)};
...
//	  myfile1.open ("rrtGPU_time.txt");
  ...
  status = cudaHostGetDevicePointer(&polygons_dev,polygons_,0);  
	  ...
	  //rfunc();	
	 fprintf(fp, " %d ", num_obstacles);
	 fprintf(fp, " %lf ", o_den/10000);
	 printf(" %lf ", o_den/10000);
