...
route_layer make_route_layer(int batch, int n, int *input_layers, int *input_sizes, int groups, int group_id)
{
  ...
  //  l.output = (float*)xcalloc(outputs * batch, sizeof(float));
    status = cudaMallocHost(&l.delta,outputs*sizeof(float));
    status = cudaMallocHost(&l.output,outputs*sizeof(float));
    l.forward = forward_route_layer;
    l.backward = backward_route_layer;
    #ifdef GPU
    l.forward_gpu = forward_route_layer_gpu;
    l.backward_gpu = backward_route_layer_gpu;
    status = cudaHostGetDevicePointer(&l.delta_gpu,l.delta,0); 
