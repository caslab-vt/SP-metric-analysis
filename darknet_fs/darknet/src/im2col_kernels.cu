...
#define CUDA_KERNEL_LOOP(i, n,offset_b, offset_grid) \
  for (int i = blockIdx.x * blockDim.x+ offset_b + threadIdx.x; \
       i < (n); \
       i += offset_grid)
...
__global__ void im2col_gpu_kernel_ext(const int n, const float* data_im,
    ...
    const int height_col, const int width_col,
    float* data_col,int offset_b, int offset_grid) {
    CUDA_KERNEL_LOOP(index, n,offset_b,offset_grid) {
    ...
    float* data_col)
{
    ...
    int grid_size = CAFFE_GET_BLOCKS(num_kernels);
    int offset_grid = grid_size * CAFFE_CUDA_NUM_THREADS;
    int extra_grid = grid_size %4;
    grid_size = grid_size / 4;
    int loopcount = 0;
    int offset_b = 0;
    int stream_id = 0;
    cudaDeviceSynchronize();
    for (loopcount = 0; loopcount < grid_size; loopcount++)
    {
		offset_b = loopcount * 4* CAFFE_CUDA_NUM_THREADS;
		stream_id = loopcount%4;
	    im2col_gpu_kernel_ext << <4,
		CAFFE_CUDA_NUM_THREADS,0,get_cuda_stream_32(stream_id) >> >(
		    num_kernels, data_im, height, width, kernel_h, kernel_w, pad_h,
		    pad_w, stride_h, stride_w, dilation_h, dilation_w, height_col,
		    width_col, data_col,offset_b,offset_grid);
    }
    if(extra_grid)
	{
	offset_b = loopcount * 4* CAFFE_CUDA_NUM_THREADS;
		stream_id = loopcount%4;
	    im2col_gpu_kernel_ext << <extra_grid,
		CAFFE_CUDA_NUM_THREADS,0,get_cuda_stream_32(stream_id) >> >(
		    num_kernels, data_im, height, width, kernel_h, kernel_w, pad_h,
		    pad_w, stride_h, stride_w, dilation_h, dilation_w, height_col,
		    width_col, data_col,offset_b,offset_grid);
