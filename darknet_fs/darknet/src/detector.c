...
/*	create_mask_on_image(im, dets, nboxes, thresh, names, alphabet, l.classes,output_filename);*/
...
masking test_detector_init(int letter_box, int benchmark_layers)
{
	printf("inside test Detector0init \n");
	char *datacfg = "/home/nvidia/darknet_fs/darknet/cfg/coco.data";
	char *cfgfile = "/home/nvidia/darknet_fs/darknet/cfg/yolov4-tiny.cfg";
	char *weightfile = "/home/nvidia/darknet_fs/darknet/yolov4-tiny.weights";
	masking masking1;
	masking1.i=0;
	printf("%d",masking1.i);
    list *options = read_data_cfg(datacfg);
    printf("inside test Detector \n");
    char *name_list = "/home/nvidia/darknet_fs/darknet/data/coco.names"; //option_find_str(options, "names", "data/names.list");
    printf("inside test Detector2 \n");
    int names_size = 0;
    printf("Name list %s\n",name_list);
    masking1.names = get_labels_custom(name_list, &names_size); //get_labels(name_list);
    printf("inside test Detector3 \n");

    masking1.alphabet = load_alphabet();
    printf("inside test Detector4 \n");
    masking1.net = parse_network_cfg_custom(cfgfile, 1, 1); // set batch=1
    printf("inside test Detector5 \n");
    if (weightfile) {
        load_weights(&masking1.net, weightfile);
    }
    if (masking1.net.letter_box) letter_box = 1;
    masking1.net.benchmark_layers = benchmark_layers;
    fuse_conv_batchnorm(masking1.net);
    calculate_binary_weights(masking1.net);
    if (masking1.net.layers[masking1.net.n - 1].classes != names_size) {
        printf("\n Error: in the file %s number of names %d that isn't equal to classes=%d in the file %s \n",
            name_list, names_size, masking1.net.layers[masking1.net.n - 1].classes, cfgfile);
        if (masking1.net.layers[masking1.net.n - 1].classes > names_size) getchar();
    }
    return masking1;
	
}

detection_with_class* test_detector(masking masking1,image im, float thresh,
    float hier_thresh, int dont_show, int save_labels, int letter_box, int benchmark_layers)
{
    printf("inside test Detector 0 \n");
/*	char *datacfg = "/home/nvidia/darknet_fs/darknet/cfg/coco.data";*/
/*	char *cfgfile = "/home/nvidia/darknet_fs/darknet/cfg/yolov4-tiny.cfg";*/
/*	char *weightfile = "/home/nvidia/darknet_fs/darknet/yolov4-tiny.weights";*/
/*    list *options = read_data_cfg(datacfg);*/
...
/*        image im = load_image(input, 0, 0, net.c);*/
        image sized;
        if(letter_box) sized = letterbox_image(im, masking1.net.w, masking1.net.h);
        else sized = resize_image(im, masking1.net.w, masking1.net.h);

        layer l = masking1.net.layers[masking1.net.n - 1];
        int k;
        for (k = 0; k < masking1.net.n; ++k) {
            layer lk = masking1.net.layers[k];
            if (lk.type == YOLO || lk.type == GAUSSIAN_YOLO || lk.type == REGION) {
                l = lk;
                printf(" Detection layer: %d - type = %d \n", k, l.type);
            }
        ...
        //time= what_time_is_it_now();
        double time = get_time_point();
        network_predict(masking1.net, X);
        //network_predict_image(&masking1.net, im); letterbox = 1;
        printf("%s: Predicted in %lf milli-seconds.\n", input, ((double)get_time_point() - time) / 1000);
        //printf("%s: Predicted in %f seconds.\n", input, (what_time_is_it_now()-time));

        int nboxes = 0;
//getting all the boxes
        detection *dets = get_network_boxes(&masking1.net, im.w, im.h, thresh, hier_thresh, 0, 1, &nboxes, letter_box);
        if (nms) {
            if (l.nms_kind == DEFAULT_NMS) do_nms_sort(dets, nboxes, l.classes, nms);
            else diounms_sort(dets, nboxes, l.classes, nms, l.nms_kind, l.beta_nms);
        }

    	printf("inside test Detector boxes \n");
	...
	int compare_by_lefts(const void *a_ptr, const void *b_ptr) {
    	...
    	printf("inside test Detector boxes2 \n");
	...
	int compare_by_probs(const void *a_ptr, const void *b_ptr) {
    	...
    	printf("inside test Detector boxe3 \n");
	 int selected_detections_num;
	 detection_with_class* selected_detections = get_actual_detections(dets, nboxes, thresh, &selected_detections_num, masking1.names);
	 qsort(selected_detections, selected_detections_num, sizeof(*selected_detections), compare_by_lefts);
	 qsort(selected_detections, selected_detections_num, sizeof(*selected_detections), compare_by_probs);
    	printf("inside test Detector boxes4 \n");
	selected_detections->n_box=nboxes; //do this in masking func
	int ext_output=0;
        draw_detections_v3(im, dets, nboxes, thresh, masking1.names, masking1.alphabet, l.classes, ext_output);
    printf("inside test Detector6 \n");
    	//cv::Mat mask = cv::Mat::ones(im.h,im.w,CV_8U);
/*	cv::Mat *mask_ptr =new cv::Mat::ones(im.h,im.w,CV_8U);*/
/*	cv::Mat &mask = *mask_ptr;*/
/*	mask=create_mask_on_image(im, dets, nboxes, thresh, masking1.names, masking1.alphabet, l.classes,output_filename);*/
/*        save_image(im, "predictions");*/
/*        if (!dont_show) {*/
/*            show_image(im, "predictions");*/
/*        }*/
        ...
        if (json_file) {
            ...
            ++json_image_id;
            json_buf = detection_to_json(dets, nboxes, l.classes, masking1.names, json_image_id, input);
...
/*    free_list_contents_kvp(masking1.options);*/
/*    free_list(masking1.options);*/
...
/*    printf("inside test Detector10 \n");*/
    //return mask;
    return (detection_with_class*)selected_detections;
}
...
/*    else if (0 == strcmp(argv[2], "multi_test")) test_detector_multiple_images(datacfg, cfg, weights, filename, thresh, hier_thresh, dont_show, ext_output, save_labels, outfile, letter_box, benchmark_layers);*/
