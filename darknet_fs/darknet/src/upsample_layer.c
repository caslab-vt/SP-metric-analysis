...
layer make_upsample_layer(int batch, int w, int h, int c, int stride)
{
  ...
  //  l.delta = (float*)xcalloc(l.outputs * batch, sizeof(float));
   // l.output = (float*)xcalloc(l.outputs * batch, sizeof(float));
    cudaMallocHost(&l.output,l.outputs*sizeof(float));
    cudaMallocHost(&l.delta,l.outputs*sizeof(float));
 ...
 //   l.delta_gpu =  cuda_make_array(l.delta, l.outputs*batch);
  ///  l.output_gpu = cuda_make_array(l.output, l.outputs*batch);
    cudaHostGetDevicePointer(&l.delta_gpu,l.delta,0);
    cudaHostGetDevicePointer(&l.output_gpu,l.output,0);
    #endif
...
void forward_upsample_layer_gpu(const layer l, network_state state)
{
    ...
    //cudaMemsetAsync(l.output_gpu,0,l.outputs,get_cuda_stream());
    if(l.reverse){
