...
char *get_label_names(char* datacfg)
{
	char** names = xcalloc(1000,sizeof(char*));
	FILE *file = fopen(datacfg, "r");
	if(file == 0) file_error(datacfg);
	char *line;
	int nu = 0;
...
void depth_estimation_test(char *cfgfile, char *weightfile, char *folder_name)
{

   FILE *fp;

   fp = fopen("depth_time.txt", "w+");
    ...
    int count = 0;
    double total_time = 0;
    int depth_deadline_missed = 0;
    if(d)
    {
	    while((dir = readdir(d)) != NULL){
		filename[0] = '\0';
		output_filename[0] = '\0';
		if((strstr(dir->d_name,ext)) || (strstr(dir->d_name,ext_png)))
		{
			strcat(filename,folder_name);
			strcat(filename,dir->d_name);
			strcat(output_filename,output_folder);
			strcat(output_filename,dir->d_name);
		}
		else
			continue;
		if(filename){
		...
		image im_out = float_to_image_scaled(224,224,1,predictions);
		save_image_CV(im_out,output_filename);
		if(0)
		{
			show_image(im_out, "test_out_2");
			wait_until_press_key_cv();
			destroy_all_windows_cv();
		}
		double time_diff = ((double)get_time_point() - time) / 1000;
	        printf("%s: Depth calculated in %lf milli-seconds.\n", input, time_diff);
   		fprintf(fp,"%f",  time/1e6);
   		fprintf(fp, " ");
   		fprintf(fp,"%f",  time_diff/1000);
   		fprintf(fp, "\n");
		...
		//if (filename) break;
		  int sleep_val = 1000.0-time_diff;
		  if(sleep_val > 0)
			usleep((sleep_val%1000)*1000);
		  	 else
		{
			printf("depth estimation Deadline missed \n");		
			depth_deadline_missed++;
		}


	    }
     }
     printf("Average Time in %lf milli-seconds.\n", total_time/count);
   fclose(fp);

    free_network(net);
}

void try_test_classifier(char *datacfg, char *cfgfile, char *weightfile, char *filename)
{
    network net = parse_network_cfg_custom(cfgfile, 1, 0);
    if(weightfile){
        load_weights(&net, weightfile);
    ...
    calculate_binary_weights(net);
    int top = 3;
    char **names = get_label_names(datacfg);
    clock_t time;
...
void run_classifier(int argc, char **argv)
{
    ...
    printf("argc = %d argv = %s",argc,argv[2]);
    if ((argc >5) && (0==strcmp(argv[2], "depth")))
    {
	 char *cfg = argv[3];
    	 char *weights = argv[4];
    	 char *filename = argv[5];
	 if ((argc >6) && (0==strcmp(argv[6], "pytorch_weights")))
	 	pytorch_weights = 1;
	 else
		pytorch_weights = 0;
	 depth_estimation_test(cfg,weights,filename); //for depth estimation test
	 return;
    ...
    if(0==strcmp(argv[2], "predict")) predict_classifier(data, cfg, weights, filename, top);
     else if(0==strcmp(argv[2], "test")) try_test_classifier(data,cfg, weights, filename);
