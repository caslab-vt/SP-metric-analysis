...
void free_layer_custom(layer l, int keep_cudnn_desc)
{
    ...
    if (l.biases)             cudaFreeHost(l.biases), l.biases = NULL; 
    if (l.type == DEPTHWISE_CONVOLUTIONAL)
	{
	      if (l.bias_updates)       cudaFreeHost(l.bias_updates), l.bias_updates = NULL;
	      if (l.scales)             cudaFreeHost(l.scales), l.scales = NULL;
              if (l.scale_updates)      cudaFreeHost(l.scale_updates), l.scale_updates = NULL;	
	}
    else
	{
    ...
    if (l.weights)            cudaFreeHost(l.weights), l.weights = NULL;
    if (l.type == DEPTHWISE_CONVOLUTIONAL)
	if (l.weight_updates)     cudaFreeHost(l.weight_updates), l.weight_updates = NULL;
    else
...
#endif  // GPU
    if (l.delta)              cudaFreeHost(l.delta), l.delta = NULL;
    if (l.output)             cudaFreeHost(l.output), l.output = NULL;
   ...
   // if (l.rolling_mean)       cudaFreeHost(l.rolling_mean), l.rolling_mean = NULL;//old
   // if (l.rolling_variance)   cudaFreeHost(l.rolling_variance), l.rolling_variance = NULL;//old
          if (l.x)                  free(l.x);
          if (l.x_norm)             free(l.x_norm);
    }
    else
    {	
	  if (l.mean)               cudaFreeHost(l.mean), l.mean = NULL;
  	  if (l.variance)           cudaFreeHost(l.variance), l.variance = NULL;
          if (l.mean_delta)         cudaFreeHost(l.mean_delta), l.mean_delta = NULL;
   	  if (l.variance_delta)     cudaFreeHost(l.variance_delta), l.variance_delta = NULL;
          if (l.rolling_mean)       cudaFreeHost(l.rolling_mean), l.rolling_mean = NULL;//old
          if (l.rolling_variance)   cudaFreeHost(l.rolling_variance), l.rolling_variance = NULL;//old
          if (l.x)                  cudaFreeHost(l.x);
          if (l.x_norm)             cudaFreeHost(l.x_norm);
    }
    if (l.m)                  free(l.m);
    if (l.v)                  free(l.v);
