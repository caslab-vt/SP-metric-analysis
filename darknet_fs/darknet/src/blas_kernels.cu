...
__global__ void scale_bias_kernel(float *output, float *scale, int batch, int filters, int spatial, int current_size,int offset)
{
    const int index = blockIdx.x*blockDim.x + offset + threadIdx.x;
...
void scale_bias_gpu(float *output, float *scale, int batch, int filters, int spatial)
{
    ...
    int num_blocks = get_number_of_blocks(current_size, BLOCK);
	...
	scale_bias_kernel << <4, BLOCK, 0, get_cuda_stream_32(stream_id) >> >(output, scale, batch, filters, spatial, current_size,offset);	
    }
    if(extra_blocks)
    {
	stream_id = loopcount%4;
	offset = loopcount * 4 * BLOCK;
	scale_bias_kernel << <extra_blocks, BLOCK, 0, get_cuda_stream_32(stream_id) >> >(output, scale, batch, filters, spatial, current_size,offset);	
    }  
    CHECK_CUDA(cudaPeekAtLastError());
}
...
__global__ void add_bias_kernel(float *output, float *biases, int batch, int filters, int spatial, int current_size,int offset)
{
    const int index = blockIdx.x*blockDim.x + offset + threadIdx.x;
    if (index >= current_size) return;
...
void add_bias_gpu(float *output, float *biases, int batch, int filters, int spatial)
{
	...
	for (loopcount = 0; loopcount < num_blocks;loopcount++)
	{
		offset = loopcount * 4*BLOCK;
		stream_id = loopcount%4;
		add_bias_kernel << <4, BLOCK, 0, get_cuda_stream_32(stream_id) >> > (output, biases, batch, filters, spatial, current_size,offset);	
	}
	//cudaDeviceSynchronize();
	if(extra_blocks)
	{
		stream_id = loopcount%4;
		offset = loopcount * 4*BLOCK;
		add_bias_kernel << <extra_blocks, BLOCK, 0, get_cuda_stream_32(stream_id) >> > (output, biases, batch, filters, spatial, current_size,offset);
	}
...
__global__ void normalize_kernel(int N, float *x, float *mean, float *variance, int batch, int filters, int spatial, int offset)
{
    const int index = blockIdx.x*blockDim.x + offset + threadIdx.x;
    if (index >= N) return;
    int f = (index / spatial) % filters;
...
extern "C" void normalize_gpu(float *x, float *mean, float *variance, int batch, int filters, int spatial)
{
	...
	offset = loopcount * 4*BLOCK;
	stream_id = loopcount%4;
	normalize_kernel << <4, BLOCK, 0, get_cuda_stream_32(stream_id) >> >(current_size, x, mean, variance, batch, filters, spatial,offset);	
    }
    if(extra_blocks)
    {
	stream_id = loopcount%4;
	offset = loopcount * 4 * BLOCK;
	normalize_kernel << <extra_blocks, BLOCK, 0, get_cuda_stream_32(stream_id) >> >(current_size, x, mean, variance, batch, filters, spatial,offset);	
    }  
    CHECK_CUDA(cudaPeekAtLastError());
...
__global__ void simple_copy_kernel(int size, float *src, float *dst,int offset)
{
...
extern "C" void simple_copy_ongpu(int size, float *src, float *dst)
{
	...
	for (loopcount = 0; loopcount < num_blocks;loopcount++)
	{
		offset = loopcount * 4*BLOCK;
		stream_id = loopcount%4;
		simple_copy_kernel << <4, BLOCK, 0, get_cuda_stream_32(stream_id) >> >(size, src, dst,offset);		
	}
	if(extra_blocks)
	{
		stream_id = loopcount%4;
		offset = loopcount * 4*BLOCK;
		simple_copy_kernel << <extra_blocks, BLOCK, 0, get_cuda_stream_32(stream_id) >> >(size, src, dst,offset);		
	}
...
__global__ void shortcut_singlelayer_simple_kernel(int size, int src_outputs, int batch, int n, int *outputs_of_layers_gpu, float **layers_output_gpu, float *out, float *in, float *weights_gpu, int nweights, WEIGHTS_NORMALIZATION_T weights_normalization, int offset)
{
    const int id = blockIdx.x*blockDim.x + offset + threadIdx.x;
    if (id >= size) return;
...
extern "C" void shortcut_multilayer_gpu(int src_outputs, int batch, int n, int *outputs_of_layers_gpu, float **layers_output_gpu, float *out, float *in, float *weights_gpu, int nweights, WEIGHTS_NORMALIZATION_T weights_normalization)
{
	...
	for (loopcount = 0; loopcount < num_blocks;loopcount++)
	{
		offset = loopcount * 4*BLOCK;
		stream_id = loopcount%4;
		shortcut_singlelayer_simple_kernel << <4, BLOCK, 0, get_cuda_stream_32(stream_id) >> > (size, src_outputs, batch, n, outputs_of_layers_gpu, layers_output_gpu, out, in, weights_gpu, nweights, weights_normalization,offset);	
	}
	if(extra_blocks)
	{
		stream_id = loopcount%4;
		offset = loopcount * 4 * BLOCK;
		shortcut_singlelayer_simple_kernel << <extra_blocks, BLOCK, 0, get_cuda_stream_32(stream_id) >> > (size, src_outputs, batch, n, outputs_of_layers_gpu, layers_output_gpu, out, in, weights_gpu, nweights, weights_normalization,offset);	
	}  	
...
__global__ void upsample_kernel(size_t N, float *x, int w, int h, int c, int batch, int stride, int forward, float scale, float *out,int offset)
{
    size_t i = blockIdx.x * blockDim.x + offset + threadIdx.x;
    if (i >= N) return;
    int out_index = i;
...
extern "C" void upsample_gpu(float *in, int w, int h, int c, int batch, int stride, int forward, float scale, float *out)
{
	...
	for (loopcount = 0; loopcount < num_blocks;loopcount++)
	{
		offset = loopcount * 4*BLOCK;
		stream_id = loopcount%4;
		upsample_kernel << <4, BLOCK, 0, get_cuda_stream_32(stream_id) >> > (size, in, w, h, c, batch, stride, forward, scale, out,offset);		
	}
	//cudaDeviceSynchronize();
	if(extra_blocks)
	{
		stream_id = loopcount%4;
		offset = loopcount * 4*BLOCK;
		upsample_kernel << <extra_blocks, BLOCK, 0, get_cuda_stream_32(stream_id) >> > (size, in, w, h, c, batch, stride, forward, scale, out,offset);		
	}
