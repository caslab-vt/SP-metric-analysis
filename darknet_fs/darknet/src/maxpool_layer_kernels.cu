...
__global__ void forward_maxpool_layer_kernel(int n, int in_h, int in_w, int in_c, int stride_x, int stride_y, int size, int pad, float *input, float *output, int *indexes, int offset)
{
    ...
    int id = blockIdx.x * blockDim.x + offset + threadIdx.x;
    if(id >= n) return;
...
extern "C" void forward_maxpool_layer_gpu(maxpool_layer layer, network_state state)
{
	...
	int extra_grid = gridSize %4;
	gridSize = gridSize / 4;
	int offset =0;
	int stream_id = 0;
	int loopcount = 0;
	cudaDeviceSynchronize();
	for ( loopcount = 0; loopcount < gridSize;loopcount++)
	{
		offset = loopcount * 4 *BLOCK;
		stream_id = loopcount%4;
       		forward_maxpool_layer_kernel << <4, BLOCK, 0, get_cuda_stream_32(stream_id) >> > (n, layer.h, layer.w, layer.c, layer.stride_x, layer.stride_y, layer.size, layer.pad, state.input, layer.output, layer.indexes_gpu,offset);	
	}
	//cudaDeviceSynchronize();
	if(extra_grid)
	{
		stream_id = loopcount%4;
		offset = loopcount * 4*BLOCK;
		forward_maxpool_layer_kernel << <extra_grid, BLOCK, 0, get_cuda_stream_32(stream_id) >> > (n, layer.h, layer.w, layer.c, layer.stride_x, layer.stride_y, layer.size, layer.pad, state.input, layer.output, layer.indexes_gpu,offset);
	}
        CHECK_CUDA(cudaPeekAtLastError());

        if (layer.maxpool_zero_nonmax) {
            forward_zero_nonmax_kernel << <cuda_gridsize(n), BLOCK, 0, get_cuda_stream() >> > (n, state.input, layer.output);
            CHECK_CUDA(cudaPeekAtLastError());
        }
    }
