...
__global__ void activate_array_leaky_kernel(float *x, int n,int offset)
{
...
__global__ void activate_array_relu6_kernel(float *x, int n, int offset)
{
    int index = blockIdx.x*blockDim.x + offset +threadIdx.x;
    if (index < n) {
...
extern "C" void activate_array_ongpu(float *x, int n, ACTIVATION a)
{
	...
	{
		num_blocks = get_number_of_blocks(n, BLOCK);
		int extra_blocks = num_blocks %8;
		int stream_id = 0;
		...
		{
			offset = loopcount * 8*BLOCK;
			stream_id = loopcount%2;
			activate_array_leaky_kernel << <8, BLOCK, 0, get_cuda_stream_32(stream_id) >> >(x, n,offset);		
		}
		if(extra_blocks)
		{
			stream_id = loopcount%2;
			offset = loopcount * 8*BLOCK;
			activate_array_leaky_kernel << <extra_blocks, BLOCK, 0, get_cuda_stream_32(stream_id) >> >(x, n,offset);		
		}
	}
    ...
    else if (a == RELU6)
	{
		...
		for (loopcount = 0; loopcount < num_blocks;loopcount++)
		{
			offset = loopcount * 8*BLOCK;
			stream_id = loopcount%2;
			activate_array_relu6_kernel << <8, BLOCK, 0, get_cuda_stream_32(stream_id) >> >(x, n,offset);	
		}
		if(extra_blocks)
		{
			stream_id = loopcount%2;
			offset = loopcount * 8*BLOCK;
			activate_array_relu6_kernel << <extra_blocks, BLOCK, 0, get_cuda_stream_32(stream_id) >> >(x, n,offset);		
		}
