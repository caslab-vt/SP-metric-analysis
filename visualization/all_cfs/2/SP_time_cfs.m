...
%             plot(SPi,WCMS,'-k','linewidth',2)
            ...
            yyaxis right; cla; hold on;
            plot(SPi,WCMS,'-k','linewidth',2)
                        ylabel('WCMS','Fontsize',24)
ax.FontSize = 20

            legend('S path','S path mean', 'WCMS','Fontsize',20,'Location','NorthEast')
...
ax = gca;
            ...
            yyaxis right; cla; hold on;
            plot(SPi,WCMS,'-k','linewidth',2)
            ylabel('WCMS','Fontsize',22)
            legend('S','S mean', 'WCMS','Fontsize',20,'Location','NorthEast')
            ax.FontSize = 18
...
%             ylabel('Makespan','Fontsize',24)
            legend('P(sol optimality%)','mean P', 'WCMS','Fontsize',20,'Location','NorthEast')
            ax.FontSize = 20

            subplot(7,1,7)
ax = gca;
            yyaxis left; cla; hold on;
            plot(SPi,SPm,'-g','linewidth',2)% plotted every data as plotting all was making the graph too dense to see
            plot(xlim, [1 1]*mean(SPm), '--k')                % First Horizontal Line at ‘y=4.5’
                                    ylabel('SP','Fontsize',24)
            xlim([1 13])
            yyaxis right; cla; hold on;
            plot(SPi,WCMS,'-k','linewidth',2)
            ylabel('WCMS','Fontsize',24)
            legend('SP','SP mean', 'WCMS','Fontsize',20,'Location','NorthEast')
ax.FontSize = 20